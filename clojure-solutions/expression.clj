...
(defn sumexp-eval [& args] (apply + (mapv clojure.math/exp args)))
(def sumexp (operation sumexp-eval))
(defn lse-eval [& args] (clojure.math/log (apply sumexp-eval args)))
(def lse (operation lse-eval))
...
(defn Sumexp [& operands] (OperationClass. 'sumexp sumexp-eval operands
  (fn [_ i & operands] (Sumexp (nth operands i)))
 ))
(defn LSE [& operands] (OperationClass. 'lse lse-eval operands
  (fn [_ i & operands] (Divide (Sumexp (nth operands i)) (apply Sumexp operands)))
 ))
